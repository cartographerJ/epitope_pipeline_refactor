...
# === EDIT THESE ===
n_frames = 180         # 180 = 6 sec slow spin, 36 = fast draft
width = 1382           # 28% wider than 1080 portrait
...
# Find the directory where the hidden PDB lives (the run's Structures/ folder)
base_dir = None
if protein_obj:
    check = os.getcwd()
    for _ in range(5):
...
output_dir = os.path.join(base_dir, "{}_spin_{}".format(base_name, timestamp))
...
if protein_obj is None:
    ...
    mp4_name = "{}_spin_{}.mp4".format(base_name, timestamp)
